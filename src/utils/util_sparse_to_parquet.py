...
def load_kmer_results(base_path, verbose=True):
    ...
    if not (os.path.exists(sparse_matrix_file) and os.path.exists(rows_file) and os.path.exists(cols_file)):
         ...
         if os.path.exists(binary_sparse_matrix_file) and os.path.exists(binary_rows_file) and os.path.exists(binary_cols_file):
             ...
             is_binary = True
             if verbose: print(f"Found binary k-mer results at: {base_path}_binary*")
         else:
            if verbose: print(f"Error: One or more required sparse result files not found for base path '{base_path}'.\n"
                              f"Checked for:\n"
                              f" - {base_path}_sparse.npz / _rows.txt / _cols.txt\n"
                              f" - {base_path}_binary_sparse.npz / _binary_rows.txt / _binary_cols.txt", file=sys.stderr)
            return None, None, None
    else:
        if verbose: print(f"Found standard k-mer results at: {base_path}*")

    if verbose:
        print(f"Loading sparse matrix from: {sparse_matrix_file}")
        print(f"Loading row names from: {rows_file}")
        print(f"Loading column names from: {cols_file}")
    ...
    try:
        ...
        if sparse_matrix.shape[0] != len(row_names):
             print(f"Error: Matrix row count ({sparse_matrix.shape[0]}) does not match number of row names ({len(row_names)}) in {rows_file}.", file=sys.stderr)
             return None, None, None
        if sparse_matrix.shape[1] != len(col_names):
             print(f"Error: Matrix column count ({sparse_matrix.shape[1]}) does not match number of column names ({len(col_names)}) in {cols_file}.", file=sys.stderr)
             return None, None, None

        if verbose:
            print(f"Loaded sparse matrix with shape {sparse_matrix.shape}.")
            print(f"Loaded {len(row_names)} row names and {len(col_names)} column names.")

        return sparse_matrix, row_names, col_names
    except FileNotFoundError as e:
         if verbose: print(f"Error: A required file was not found during loading: {e}", file=sys.stderr)
         return None, None, None
    except Exception as e:
        if verbose: print(f"Error loading k-mer results: {e}", file=sys.stderr)
        return None, None, None
...
def parse_arguments():
    ...
    parser.add_argument(
        "-i", "--input_base",
        required=True,
        help=("Base path of the saved sparse k-mer results. "
    ...
    parser.add_argument(
        "-o", "--output_parquet",
        required=True,
        help="Path for the output dense Parquet file."
    )
    parser.add_argument(
        "-q", "--quiet",
        action="store_true",
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Error: This script requires the 'pyarrow' library to write Parquet files.", file=sys.stderr)
        print("Please install it, for example using: pip install pyarrow", file=sys.stderr)
        sys.exit(1)
    ...
    if verbose:
        print("--- Sparse K-mer to Dense Parquet Converter ---")
        print(f"Input Base Path: {args.input_base}")
        print(f"Output Parquet File: {args.output_parquet}")
    ...
    if sparse_matrix is None:
        print("Failed to load sparse data. Exiting.", file=sys.stderr)
        sys.exit(1)

    # 2. Convert to Dense DataFrame
    if verbose:
        print("\nConverting sparse matrix to dense Pandas DataFrame...")
        print(f"(Matrix dimensions: {sparse_matrix.shape[0]} rows x {sparse_matrix.shape[1]} columns)")
        num_elements = sparse_matrix.shape[0] * sparse_matrix.shape[1]
        if num_elements > 10**8: # Warn if potentially large
             print("Warning: Matrix dimensions are large. Conversion to dense format might require significant memory.", file=sys.stderr)
    ...
    try:
        ...
        if verbose:
            ...
            print(f"Dense DataFrame created successfully. Estimated memory usage: {dense_mem_mb:.2f} MB")

    except MemoryError:
        print("\nERROR: A MemoryError occurred while converting the sparse matrix to a dense DataFrame.", file=sys.stderr)
        print("The dataset is too large to fit into memory in a dense format.", file=sys.stderr)
        print("Consider processing the sparse matrix directly if possible, or using a machine with more RAM.", file=sys.stderr)
        sys.exit(1)
    except Exception as e:
    ...
    if verbose:
        ...
        print(f"Using compression: {args.compression if args.compression else 'None'}")
    ...
    try:
        ...
        if verbose: print("Parquet file saved successfully.")
