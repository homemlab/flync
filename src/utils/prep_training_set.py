...
def prep_training_data(
    ...
    base["id"] = base.apply(
        lambda x: (
            f"{x['transcript_id']}_{x['exon_number']}_{x['Start']}_{x['End']}"
            if pd.notnull(x["exon_number"])
            else f"{x['transcript_id']}_{x['Start']}_{x['End']}"
        ),
        axis=1,
    )
    ...
    assert len(row_names) == len(
        set(row_names)
    ), f"Duplicate row names found: {len(row_names) - len(set(row_names))} duplicates"
...
def main():
    ...
    args.add_argument(
        "--target", type=str, default="ncr", choices=["ncr", "pcg"], help="Target variable"
    )
    args.add_argument(
        "--base_parquet_path", type=str, required=True, help="Path to base parquet file"
    )
    args.add_argument(
        "--bwq_parquet_path", type=str, required=True, help="Path to bwq parquet file"
    )
    args.add_argument(
        "--cpat_parquet_path", type=str, required=True, help="Path to cpat parquet file"
    )
    args.add_argument(
        "--mfe_parquet_path", type=str, required=True, help="Path to mfe parquet file"
    )
    args.add_argument(
        "--kmer_base_path",
        type=str,
        required=True,
        help="Base path for kmer sparse matrix files (no extension)",
    )
    args.add_argument(
        "--output_path", type=str, required=True, help="Output file path for the training data"
    )
    args.add_argument(
        "--write_dense",
        action="store_true",
        help="Try to write dense matrix to disk. WARNING: This may take a long time and use a lot of memory.",
    )
    args.add_argument(
        "--write_sparse",
        action="store_true",
        help="Write output as a sparse parquet file (default is dense if --write_dense is set, otherwise sparse).",
    )
    args.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    args.add_argument(
        "--no_tfidf", action="store_true", help="Disable TF-IDF transformation of k-mer counts."
    )
