...
try:
    ...
    logging.info(f"Error importing cpmodule: {e}")
    logging.info("Please ensure cpmodule is installed and accessible.")
    sys.exit(1)
...
def check_dependencies(rscript_check: bool = True):
    ...
    if rscript_check:
        if not shutil.which("Rscript"):
            ...
            logging.error(
                "Please install R and ensure 'Rscript' is in your system PATH."
            )
...
def run_cpat_calculation(
    ...
    try:
        ...
        logging.warning(
            f"Failed to index reference genome (may cause issues if seq_from_bed requires it): {e}"
        )
    ...
    try:
        ...
        if "Name" not in df_bed.columns:
            if "name" in df_bed.columns:  # Check for lowercase 'name'
                ...
                logging.warning(
                    "BED file lacks 'Name' column (column 4). Using index as transcript ID."
                )
                # Create a unique ID based on chrom/start/end/strand if possible
                df_bed["Name"] = df_bed.apply(
                    lambda r: f"{r.Chromosome}:{r.Start}-{r.End}({r.get('Strand', '+')})_idx{r.name}",
                    axis=1,
                )

        if "Strand" not in df_bed.columns:
            logging.warning(
                "BED file lacks 'Strand' column (column 6). Assuming '+' strand."
            )
        ...
        # Default 'Score' to 0
        if "Score" not in df_bed.columns:
            df_bed["Score"] = 0
        # Default ThickStart/End to Start/End
        if "ThickStart" not in df_bed.columns:
            df_bed["ThickStart"] = df_bed["Start"]
        if "ThickEnd" not in df_bed.columns:
            df_bed["ThickEnd"] = df_bed["End"]
        # Default ItemRGB to black
        if "ItemRGB" not in df_bed.columns:
            df_bed["ItemRGB"] = "0,0,0"
        # Calculate BlockCount, BlockSizes, BlockStarts if missing (assuming single exon if no info)
        if "BlockCount" not in df_bed.columns:
            df_bed["BlockCount"] = 1
        if "BlockSizes" not in df_bed.columns:
            df_bed["BlockSizes"] = df_bed["End"] - df_bed["Start"]
        if "BlockStarts" not in df_bed.columns:
            df_bed["BlockStarts"] = 0
    ...
    # Create progress manager and progress bars
    ...
    # Progress bar for BED line reconstruction
    bed_reconstruction_progress = progress_manager.create_bar(
        total=len(df_bed), desc="Reconstructing BED entries"
    )
    ...
    for _, row in df_bed.iterrows():
        ...
        try:
            line = "\t".join(
                map(
                    ...
                    [
                        ...
                        row.get("Score", 0),
                        row.get("Strand", "+"),
                        row.get("ThickStart", row["Start"]),
                        row.get("ThickEnd", row["End"]),
                        row.get("ItemRGB", "0,0,0"),
                        row.get("BlockCount", 1),
            ...
            logging.error(
                f"Missing expected BED column for reconstruction: {e} in row: {row.to_dict()}"
            )
            error_count += 1
        except Exception as e:
            logging.error(
                f"Error reconstructing BED line for row {row.get('Name', 'Unknown')}: {e}"
            )
            error_count += 1
        finally:
            bed_reconstruction_progress.update(1)

    # Close BED reconstruction progress bar
    bed_reconstruction_progress.close()
    
    # Progress bar for ORF processing
    orf_processing_progress = progress_manager.create_bar(
        total=len(bed_lines), desc="Processing transcripts for ORFs"
    )
    ...
    for i, bed_line in enumerate(bed_lines):
        ...
        try:
            ...
            if not seq:
                logging.warning(
                    f"Could not extract sequence for {transcript_id}. Skipping ORF search for this transcript."
                )
            ...
            rna_lengths_data.append(
                {"Transcript_ID": transcript_id, "Transcript_Len": RNA_len}
            )
            ...
            for orf in ORFs:
                ...
                orf_end_rel = orf[
                    3
                ]  # End coord is inclusive in this context? Check cpmodule doc. Usually python is exclusive. Assume 0-based exclusive for end.
                ...
                if len(orf_seq) != orf_len:
                    logging.warning(
                        f"ORF length mismatch for {transcript_id} ORF {orf_sn}. Reported: {orf_len}, Sequence: {len(orf_seq)}"
                    )
                if orf_len != (orf_end_rel_0based - orf_start_rel_0based):
                    logging.warning(
                        f"ORF coordinate mismatch for {transcript_id} ORF {orf_sn}. Len: {orf_len}, Coords: {orf_start_rel_0based}-{orf_end_rel_0based}"
                    )
                ...
                hexamer_score = FrameKmer.kmer_ratio(
                    orf_seq, 6, 3, coding_hex, noncoding_hex
                )
                ...
                orf_info_data.append(
                    [
                        ...
                        orf_strand,
                        orf_frame,
                        orf_start_rel_0based
                        + 1,  # Convert to 1-based start for output convention
        ...
        #     error_count += 1
            ...
            orf_processing_progress.update(1)

    logging.info(f"Finished ORF finding.")
    logging.info(
        f"- Found ORFs for {len(orf_info_data)} candidates across transcripts."
    )
    logging.info(
        f"- Entries with no ORFs found or seq errors: {len(no_orf_entries) + error_count}"
    )
    ...
    if df_transcript_lengths.empty:
        ...
        df_transcript_lengths = df_transcript_lengths.drop_duplicates(
            subset=["Transcript_ID"]
        )
        all_transcripts_with_len_df = pd.merge(
            all_transcript_ids_master_df,
            df_transcript_lengths,
            on="Transcript_ID",
            how="left",
        )
    ...
    if not orf_info_data:
        logging.info(
            "No valid ORFs were identified from any transcript sequences. "
        ...
        final_df = final_df.reindex(
            columns=final_columns, fill_value=np.nan
        )  # Ensure order and all columns

        logging.info(
            f"Saving final aggregated results per transcript to {output_parquet_path}"
        )
        try:
            ...
            logging.error(
                f"Failed to save results to Parquet file (no ORFs found scenario): {e_save}"
            )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        logging.info(
            f"Writing intermediate ORF info for R script to {orf_info_tsv_path}"
        )
        try:
            ...
            df_r_input = df_orf_input.rename(
                columns={
                    "ORF_ID": "ID",
                    "Transcript_Len": "mRNA",
                    "ORF_Len": "ORF",
            ...
            if not all(col in df_r_input.columns for col in r_input_cols):
                ...
                raise ValueError(
                    f"Internal error: Missing columns required for R input: {missing_cols}"
                )
            ...
            logging.error(
                f"Failed to write intermediate ORF info TSV for R script: {e}"
            )
            raise

        # Close ORF processing progress bar
        orf_processing_progress.close()
        
        # 6. Run coding_prediction (which calls the R script)
        logging.info(
            "Calculating coding probability using the logit model via Rscript..."
        )

        # Create progress bar for R prediction step
        r_prediction_progress = progress_manager.create_bar(
            total=None, desc="Running R prediction script"
        )
        ...
        try:
            ...
            r_prediction_progress.update(1)

            if not os.path.exists(orf_prob_tsv_path):
                raise FileNotFoundError(
                    f"R script completed but did not create the expected output file: {orf_prob_tsv_path}"
                )
            if os.path.getsize(orf_prob_tsv_path) == 0:
                raise ValueError(
                    f"R script created an empty output file: {orf_prob_tsv_path}"
                )
            ...
            if (
                "ID" not in results_df_raw.columns
                or "Coding_prob" not in results_df_raw.columns
            ):
                logging.error(
                    f"CPAT output TSV {orf_prob_tsv_path} is missing expected columns ('ID', 'Coding_prob'). Found: {results_df_raw.columns.tolist()}"
                )
                ...
                if "Coding_prob" in results_df_raw.columns:  # Double check
                    ...
                    logging.info(
                        "R script execution and result loading seem successful."
                    )
                else:  # Should be caught by the column check above
                    logging.error(
                        f"CPAT output TSV {orf_prob_tsv_path} read but 'Coding_prob' column is missing."
                    )

        except FileNotFoundError as e:
            logging.error(f"Error related to R script output file: {e}")
            logging.error(
                "This likely means the R script failed or did not produce the expected output."
            )
            logging.error(
                "Check R installation, PATH, and potential R errors if logged elsewhere."
            )
            # cpat_scores_successfully_loaded remains False
        except (
            ValueError
        ) as e:  # For empty file or issues with content like missing columns
            logging.error(
                f"Error processing R script output file ({orf_prob_tsv_path}): {e}"
            )
            # cpat_scores_successfully_loaded remains False
        except Exception as e:
            logging.error(
                f"General error during coding_prediction (R script execution or output processing): {e}",
                exc_info=True,
            )
            # cpat_scores_successfully_loaded remains False
            if not os.path.exists(orf_prob_tsv_path):
                logging.error(
                    f"Expected R output file {orf_prob_tsv_path} was not created or accessible after error."
                )
        ...
        logging.info(
            "Combining ORF details with coding probabilities (if available)..."
        )
        if not cpat_scores_successfully_loaded or results_df_raw.empty:
            logging.warning(
                "CPAT scores ('Coding_prob') are not available or failed to load. 'Coding_prob' will be NA for all ORFs."
            )
            ...
            combined_df = df_orf_details.join(
                results_df_raw[["Coding_prob"]], how="left"
            )
            ...
            if combined_df["Coding_prob"].isnull().any():
                logging.warning(
                    "Some ORFs have missing 'Coding_prob' after join. This may be due to R script filtering or partial results."
                )
        ...
        if "Transcript_ID" not in combined_df.columns and not combined_df.empty:
            logging.error(
                "Internal Error: 'Transcript_ID' column is missing in combined_df before aggregation."
            )
            # Create an empty df with expected columns to allow merge to proceed and fill with NAs
            best_orfs_found_df = pd.DataFrame(
                columns=["Transcript_ID"]
                + [col for col in final_columns if col != "Transcript_ID"]
            )
        elif combined_df.empty:
            logging.info(
                "combined_df is empty (e.g., no ORFs passed to R or R data issue). No ORFs to aggregate."
            )
            best_orfs_found_df = pd.DataFrame(
                columns=["Transcript_ID"]
                + [col for col in final_columns if col != "Transcript_ID"]
            ...
            best_orfs_found_df = (
                combined_df_sorted.groupby("Transcript_ID").first().reset_index()
            )
    ...
    final_df_merged = pd.merge(
        ...
        best_orfs_found_df.drop(
            columns=["Transcript_Len"], errors="ignore"
        ),  # errors='ignore' in case Transcript_Len isn't in best_orfs_found_df
        on="Transcript_ID",
        how="left",
    )
    ...
    final_df_output.columns = [
        col.lower() for col in final_df_output.columns
...
def calculate_cpat_scores(
    ...
    """
    Calculate CPAT (Coding Potential Assessment Tool) scores for transcripts in a BED file.

    This function provides a simple API interface to run_cpat_calculation with sensible
    defaults for programmatic usage. It analyzes coding potential of RNA sequences by
    finding open reading frames (ORFs) and calculating Fickett scores, hexamer scores,
    and coding probabilities.

    Args:
        bed_file_path: Path to input BED file with transcript coordinates
        ref_genome_path: Path to reference genome FASTA file
        hexamer_dat_path: Path to hexamer frequency table file (e.g., fly_Hexamer.tsv)
        logit_model_path: Path to logistic regression model (.RData file)
        output_parquet_path: Optional path to save results as Parquet file
        min_orf_len: Minimum ORF length in nucleotides
        n_top_orf: Maximum number of ORF candidates per strand to consider per transcript
        start_codons: List of start codons. Defaults to ["ATG"]
        stop_codons: List of stop codons. Defaults to ["TAG", "TAA", "TGA"]
        antisense: Whether to search ORFs on the antisense strand

    Returns:
        pandas.DataFrame: CPAT results with columns:
            - transcript_id: Transcript identifier
            - coding_prob: Coding probability (0-1)
            - orf_id: Best ORF identifier
            - orf_len: ORF length
            - fickett_score: Fickett score
            - hexamer_score: Hexamer score
            - orf_strand: ORF strand
            - orf_frame: ORF reading frame
            - orf_start_transcript: ORF start position in transcript
            - orf_end_transcript: ORF end position in transcript
            - transcript_len: Transcript length

    Raises:
        FileNotFoundError: If required input files don't exist
        ValueError: If parameters are invalid
        RuntimeError: If CPAT calculation fails

    Example:
        >>> from features.cpat import calculate_cpat_scores
        >>>
        >>> # Basic usage
        >>> results = calculate_cpat_scores(
        ...     bed_file_path="transcripts.bed",
        ...     ref_genome_path="genome.fa",
        ...     hexamer_dat_path="fly_Hexamer.tsv",
        ...     logit_model_path="Fly_logitModel.RData"
        ... )
        >>> print(f"Processed {len(results)} transcripts")
        >>> print(results[['transcript_id', 'coding_prob']].head())
    """
    ...
    try:
        ...
        logging.error(f"CPAT calculation failed: {e}")
        raise
...
def main() -> int:
    ...
    parser.add_argument(
        "--start_codons", nargs="+", default=["ATG"], help="List of start codons"
    )
    parser.add_argument(
        "--stop_codons",
        nargs="+",
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Enable more detailed logging"
    )

    # Add unified progress arguments
    ...
    if "coding_prediction" not in dir(sys.modules.get("cpmodule.utils")):
        logging.error(
            "Error: cpmodule.utils.coding_prediction not found. Cannot proceed."
        )
        return 1
    ...
    try:
        ...
        cpat_results_df = run_cpat_calculation(
            bed_file_path=args.input_bed,
            ref_genome_path=args.reference_fasta,
            hexamer_dat_path=args.hexamer_table,
            logit_model_path=args.logit_model,
            output_parquet_path=args.output_parquet,
            min_orf_len=args.min_orf_length,
            n_top_orf=args.num_top_orfs,
        ...
        if not cpat_results_df.empty:
            logging.info(
                f"Mean Coding Probability: {cpat_results_df['coding_prob'].mean():.4f}"
            )
            logging.info(
                f"Median Coding Probability: {cpat_results_df['coding_prob'].median():.4f}"
            )
            logging.info("First 5 rows of the result:")
            # Use print for DataFrame display since logging doesn't handle it well
            logger.info(cpat_results_df.head().to_string())
        else:
            logging.info("Result DataFrame is empty.")

        # Verify Parquet file creation
        if (
            os.path.exists(args.output_parquet)
            and os.path.getsize(args.output_parquet) > 0
        ):
            logging.info(f"Results successfully saved to: {args.output_parquet}")
        elif os.path.exists(args.output_parquet):
            logging.warning(
                f"Output Parquet file was created but is empty: {args.output_parquet}"
            )
        else:
            logging.error(f"Output Parquet file not found at {args.output_parquet}")
            return 1
