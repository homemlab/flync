...
def get_folding_results_mp_helper_indexed(item: tuple[int, str]) -> tuple[int, float | None, str | None]:
    ...
    try:
        ...
        logging.warning(f"LinearFold folding failed for sequence at index {original_idx} (len {len(sequence)}): {e}")
        return original_idx, pd.NA, pd.NA
    except Exception as e:
        logging.error(f"An unexpected error occurred in worker for sequence at index {original_idx}: {e}")
        return original_idx, pd.NA, pd.NA
...
def calculate_all_mfe_and_structure(
    ...
    logging.info(f"Calculating MFE for {total_sequences_to_calculate} sequences using {num_processes} processes with LinearFold...")
...
def load_checkpoint_batch(filename: str, include_structure: bool, sequence_col: str) -> pd.DataFrame | None:
    ...
    try:
        ...
        if 'mfe' not in df_checkpoint.columns or (include_structure and 'structure' not in df_checkpoint.columns) or sequence_col not in df_checkpoint.columns:
            logging.warning(f"Checkpoint file '{filename}' is missing required columns. Will re-process.")
            return None
...
def write_output_file(df: pd.DataFrame, filename: str):
    ...
    try:
        ...
        if output_ext == '.csv':
            ...
            if not pyarrow_available:
                 logging.error(f"Cannot write Parquet file '{filename}' because 'pyarrow' is not installed.")
                 sys.exit(1)
            df.to_parquet(filename, engine='pyarrow', index=False)
        else:
            logging.error(f"Unsupported output file format '{output_ext}'. Please use .csv, .tsv, or .parquet.")
            sys.exit(1)
...
def process_mfe_calculations(
    ...
    try:
        ...
        if input_ext == '.csv':
            ...
            logging.error(f"Unsupported input file format '{input_ext}'.")
            sys.exit(1)
    ...
    if batch_size > 0:
        ...
        for i in range(num_expected_batches):
            ...
            if os.path.exists(batch_filename):
                ...
                if df_checkpoint is not None and len(df_checkpoint) == (batch_end_orig_idx - batch_start_orig_idx):
                    logging.info(f"Valid checkpoint loaded for rows {batch_start_orig_idx + 1}-{batch_end_orig_idx}.")
                    ...
                    if include_structure:
                        final_results_df.loc[indices_to_update, 'structure'] = df_checkpoint['structure'].values
...
def main():
    ...
    parser.add_argument("input_file", help="Path to the input file (.csv, .tsv, .parquet).")
    parser.add_argument("output_file", help="Path/base name for the output file(s).")
    parser.add_argument("-s", "--sequence_col", default="Sequence", help="Name of the column with RNA sequences.")
    parser.add_argument("--include_structure", action="store_true", help="Include dot-bracket structure in output.")
    parser.add_argument("-b", "--batch_size", type=int, default=0, help="Batch size for processing. 0 means single output. >0 enables checkpointing.")
    parser.add_argument("--num_processes", "-p", type=int, default=None, help="Number of parallel processes. Defaults to all CPU cores.")
    parser.add_argument("--log_level", choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"], default="INFO", help="Set logging level.")
    ...
    if pyarrow_available:
        ...
        logging.warning("'pyarrow' not found. Parquet support is unavailable. Install with: pip install pandas pyarrow")
