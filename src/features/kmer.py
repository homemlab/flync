...
def _dna_to_binary(dna_sequence):
    """
    Converts a DNA sequence string (ATGC) to its binary representation.
    ...
    """
    dna_sequence = dna_sequence.upper() # Ensure case-insensitivity for input DNA
    binary_seq = []
    for base in dna_sequence:
        if base == 'A' or base == 'T':
            ...
            binary_seq.append('1')
        # Note: Non-ATGC characters are effectively skipped.
...
def _generate_all_kmers(k_min, k_max, alphabet='01', verbose=True):
    ...
    t_start = time.time()
    if verbose:
        logging.info(f"Generating all possible k-mers for k={k_min}-{k_max} using alphabet '{alphabet}'...")
    for k in range(k_min, k_max + 1):
        for p in itertools.product(alphabet, repeat=k):
            all_kmers.add("".join(p))
    num_kmers = len(all_kmers)
    if verbose:
        logging.info(f"Generated {num_kmers:,} unique k-mers in {time.time() - t_start:.2f}s.")
...
def calculate_kmer_profiles(
    ...
    if not (1 <= k_min <= k_max):
        logger.error(f"Invalid k-mer range: k_min={k_min}, k_max={k_max}. k_min must be >= 1 and k_min <= k_max.")
    ...
    if return_type not in allowed_returns:
        logger.error(f"Invalid return_type '{return_type}'. Choose from {allowed_returns}.")
        raise ValueError(f"Invalid return_type '{return_type}'. Choose from {allowed_returns}.")
        ...
        try:
            ...
            while not eof:
                ...
                for _ in range(batch_size):
                    try:
                        ...
                        while current_seq_id in global_processed_ids_check:
                            if dup_counter == 1 and verbose: # Print warning only on first detection
                                print(f"Warning: Duplicate ID '{original_seq_id}' from file '{file_basename}'. Renaming to avoid collision.", file=sys.stderr)
                            current_seq_id = f"{file_basename}_{original_seq_id}_dup{dup_counter}"
                            dup_counter += 1

                        if record.seq: # Ensure sequence is not empty
                            current_batch_dna_sequences.append((current_seq_id, str(record.seq)))
                ...
                for seq_id, dna_seq_str in current_batch_dna_sequences:
                    ...
                    if not binary_seq_str and verbose: # If binary string is empty (e.g. original DNA had no ATGC)
                        print(f"Warning: Sequence ID '{seq_id}' resulted in an empty binary string. It will have no k-mers.", file=sys.stderr)
                ...
                batch_id_to_row_idx = {seq_id: i for i, seq_id in enumerate(current_batch_ids_ordered)}
                ...
                for seq_id, kmer_counts_for_seq in batch_results:
                    ...
                    for kmer, count in kmer_counts_for_seq.items():
                        ...
                        if col_idx is not None: # K-mer is in our list of all possible k-mers
                ...
                batch_matrix_csr = batch_matrix.tocsr() # Convert to CSR for efficient storage and stacking
            ...
            if not partial_matrix_files:
                if total_sequences_processed == 0:
                    raise RuntimeError("No valid sequences were processed from the input.")
            ...
            try:
                ...
                raise MemoryError("Ran out of memory while trying to load and combine batch matrices. "
                                  "Consider using a smaller --batch_size.")
            except Exception as e:
                raise RuntimeError(f"An error occurred while combining batch matrices: {e}")

            end_time_combine = time.time()
            if verbose:
                print(f"Combined all batch matrices in {end_time_combine - start_time_combine:.2f}s.")
                ...
                if final_sparse_matrix_csr.shape[0] != len(master_sequence_ids_ordered):
                    print(f"Warning: Final matrix row count ({final_sparse_matrix_csr.shape[0]}) "
                          ...
                          "This might indicate an issue with ID tracking or batch processing.", file=sys.stderr)
    ...
    if return_type == 'sparse_matrix':
        ...
        if num_elements > 10**8: # Arbitrary threshold for large matrix
            print(f"Warning: The final matrix dimensions ({final_sparse_matrix_csr.shape[0]}x{final_sparse_matrix_csr.shape[1]}) "
                  "are large. Converting to a dense DataFrame may cause a MemoryError.", file=sys.stderr)
        try:
            ...
            if verbose:
                print("\nERROR: A MemoryError occurred while creating the dense Pandas DataFrame. "
                      "The dataset is too large for a dense representation in memory.", file=sys.stderr)
...
def process_kmer_calculation(args):
    ...
    if not fasta_extensions:
        logger.error("No valid FASTA extensions provided. Use --ext (e.g., '.fa,.fasta').")
        return 1
    ...
    try:
        ...
        calc_return_type = 'dense_df' if args.output and \
                                         (args.output_format == 'dense_csv' or \
                                          args.output_format == 'dense_parquet') \
                                     else 'sparse_matrix'
        ...
        if args.output:
            ...
            if results is None and calc_return_type == 'dense_df':
                if verbose:
                    logger.error("Dense DataFrame could not be created (likely due to MemoryError). "
            ...
            if save_successful and results is not None: # Proceed only if results exist
                ...
                if args.output_format == 'sparse':
                    if calc_return_type == 'sparse_matrix': # results should be (matrix, rows, cols)
                        ...
                        if verbose: logger.info(f"Saving sparse binary k-mer results to: {output_base_tagged}_*")
                        try:
                            save_npz(sparse_matrix_file, sparse_matrix)
                            with open(rows_file, 'w') as f:
                                for item in row_names: f.write(f"{item}\n")
                            with open(cols_file, 'w') as f:
                                for item in col_names: f.write(f"{item}\n")
                            if verbose:
                        ...
                        logger.error(f"Cannot save in sparse format. Expected 'sparse_matrix' results, "
                    ...
                    if calc_return_type == 'dense_df' and isinstance(results, pd.DataFrame):
                        ...
                        logger.error(f"Cannot save as dense CSV. Expected a pandas DataFrame, "
                              f"but received type {type(results)}.")
                        save_successful = False
                    ...
                    if calc_return_type == 'dense_df' and isinstance(results, pd.DataFrame):
                        ...
                        logger.error(f"Cannot save as dense Parquet. Expected a pandas DataFrame, "
                              f"but received type {type(results)}.")
                        save_successful = False
            ...
            if results is None:
                logger.warning("Calculation completed, but no results were generated (e.g., MemoryError for dense DataFrame). "
                      "Results not saved as --output was not specified.")
            else:
                logger.info("Calculation complete. Results were not saved as --output was not specified.")
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default='sparse',
        help=("Format for saving output if --output is specified. "
