...
do
    if [ ! -d $workdir/kallisto/$i ]; then
        ...
        if [ $lay == 'SINGLE' ]; then
            ...
            kallisto quant --genomebam -g $workdir/results/new-lincRNAs.gtf -b 50 -t $threads -o $workdir/kallisto/$i -i $workdir/kallisto/new-transcripts.fa.idx -o $i -l $readlen -s $sd $workdir/data/$i/$i'_1.fastq.gz' $workdir/data/$i/$i'_1.fastq.gz'
        fi
    fi
done < $sra
echo 'Done'
