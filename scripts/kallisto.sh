...
do
    if [ ! -d $workdir/kallisto/$i ]; then
        ...
        sd=${sd%.*}
        if [ $lay == 'SINGLE' ]; then
            kallisto quant -b 25 -t $threads -o $workdir/kallisto/$i -i $workdir/kallisto/new-transcripts.fa.idx -o $i --single -l $readlen -s $sd $workdir/data/$i/$i'.fastq.gz'
        else
            kallisto quant -b 50 -t $threads -o $workdir/kallisto/$i -i $workdir/kallisto/new-transcripts.fa.idx -o $i -l $readlen -s $sd $workdir/data/$i/$i'_1.fastq.gz' $workdir/data/$i/$i'_1.fastq.gz'
        fi
    fi
done < $sra
echo 'Done'
